...
';
		...
		<div class="container">
			...
			<div class="title"><h3>'.$blog['child']['TITLE'][0]['data'].'</h3>'.gmdate('h:i a, j F Y', $blog['child']['TIME'][0]['data']).'<h3></h3></div>
			<div class="textbody">
...
if ($submission == TRUE)
{
	...
	$secret_key = "6LfgFgYTAAAAADc-q9HVB9Aqb9De7F4YlG43H7EQ";
	...
	else
	{
		...
		while ($i < $num)
		{
			...
			$diff = gmmktime(gmdate("G"),gmdate("i"),gmdate("s"),gmdate("m"),gmdate("d"),gmdate("Y")) - $date_of_commenting;
			$unit = "second";
			if ($diff > 60)
			{
				$diff = floor($diff/60);
					...
					if ($diff > 24)
					{
						...
						if ($diff > 7)
						{
							...
							if ($diff > 3)
							{
								...
								$diff = "on ";
								$unit = gmdate('l dS \of F Y h:i:s A', $date_of_commenting);
							}
						}
					}
				}
...
';
	...
	/* Show the re-captcha */
	$site_key = "6LfgFgYTAAAAAGQoOb1PyT7vPqLwOPr6jjrZp0Ou";
	echo '
